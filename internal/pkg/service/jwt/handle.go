...
func (s *Service) ParseResetPasswordJWT(ctx context.Context, value string) (*ResetPasswordClaims, error) {
	token, err := jwt.ParseWithClaims(value, &ResetPasswordClaims{}, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, ErrUnexpectedMethod
		}
		return []byte(s.cfg.JWTSecretSignUp), nil
	})
	if err != nil {
		return &ResetPasswordClaims{}, ErrInvalidToken
	}
	myClaims := token.Claims.(*ResetPasswordClaims)
	return &ResetPasswordClaims{
		Email:  myClaims.Email,
		UserId: myClaims.UserId,
	}, nil
}

func (s *Service) CreateResetPasswordJWT(ctx context.Context, claims ResetPasswordClaims) (string, error) {
	jwtSecret := []byte(s.cfg.JWTSecretSignUp)
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"Email":     claims.Email,
